...
class w2v2_predictor:
    def _create_ultrasuite(self):
        self.ultrasuite = load_dataset("./ultrasuite", data_dir = "/path/to/ultrasuite/dataset")
    ...
    def _create_dicts(self):
        ...
        def _generate_dict(self):
            ...
            if self.ultrasuite == None:
                self._create_ultrasuite()

            vocabs = self.ultrasuite.map(_extract_all_phones, batched=True, batch_size=-1, keep_in_memory=True,
                                    remove_columns=self.ultrasuite.column_names["train"])
            ...
            # save str-unicode/arpabet_phoneme-unicode (decode back to arphabet)
        ...
        try:
            ...
            if not os.path.isfile('./vocab.json'):
                #Process ultrasuite and create a new dictionary (you will have to re-finetune a model)
                _generate_dict(self)
            else:
    ...
    ## GET ultrasuite DATASET IN THE FORM READY FOR DICTIONARY CREATION AND ANALYSIS
    def _preprocess_ultrasuite(self):
        # for use with ultrasuite.map - add a unicode representation of each phone to the dataset
        ...
        try:
            ...
            self.ultrasuite = self.ultrasuite.map(to_unicode_fn, batch_size=-1, keep_in_memory=True, batched=True)

            self.ultrasuite = self.ultrasuite.map(delim_phones_fn, batch_size=-1, keep_in_memory=True, batched=True)

            self.ultrasuite = self.ultrasuite.map(speech_file_to_array_fn, remove_columns=self.ultrasuite.column_names["train"],
                                        num_proc=8)

            ## VALIDATE SHAPE
            rand_int = random.randint(0, len(self.ultrasuite["train"]))
            print("Preprocessed Shape Inspection"+"\n"+"*"*26)
            print("Index No:", rand_int)
            print("Target phones:", self.ultrasuite["train"][rand_int]["target_phones"])
            print("Input array shape:", np.asarray(self.ultrasuite["train"][rand_int]["speech"]).shape)
        except Exception as e:
            print("Preprocessing failed " + str(e))

    ## GENERATE THE PROCESSORS. IF SAVE DETECTED, GENERATE FROM THERE, ELSE CONSTRUCT FROM PREPROCESSED ultrasuite.
    def _generate_processors(self):
        ## BUILD PROCESSOR
        if (os.path.isdir('/path/to/processor/') and os.path.isfile('./vocab.json')):
            ## TOKENIZER CLASS
            self.tokenizer = Wav2Vec2CTCTokenizer(vocab_file="/path/to/vocab.json",
            ...
            ## CREATE PROCESSOR if self.ultrasuite_prepared == None:
            self._preprocess_ultrasuite()
            self.processor = Wav2Vec2Processor(feature_extractor=self.feature_extractor, tokenizer=self.tokenizer)

            ## SAVE PROCESSOR
            self.processor.save_pretrained("/path/to/processor")
        else:
            #PREPROCESS ultrasuite AND GENERATE THE MISSING FILES
            self._create_dicts()
    ...
    ## GET ultrasuite INTO ITS FINAL FORM BEFORE EVALUATING/TESTING. REQUIRES PROCESSORS AND PREPROCESSED ultrasuite
    def process_ultrasuite(self):
        ## PROCESS
        ...
        self.ultrasuite_prepared = self.ultrasuite.map(prepare_dataset, remove_columns=self.ultrasuite.column_names["train"],
                                             batch_size=8,
                                             batched=True)
    ...
    def __init__(self):
        ...
        # Datasets
        self.ultrasuite = None
        self.ultrasuite_prepared = None
    ...
    def set_model(self, ckpt):
        ...
        if ckpt is None:
            self.model = Wav2Vec2ForCTC.from_pretrained(
                "facebook/wav2vec2-large-xls-r-1b",
    ...
    def train(self, ckpt = None):
        ...
        try:
            ...
            trainer = Trainer(
                ...
                train_dataset=self.ultrasuite_prepared["train"],
                eval_dataset=self.ultrasuite_prepared["test"],
    ...
    def eval(self, ckpt = None, proc = None):
        ...
        try:
            ...
            results = self.ultrasuite["test"].map(map_to_result)
    ...
    def pred_wav_with_collapse(self, wavpath, ckpt=None, return_type="phones"):
        try:
            ...
            # Different part: convert ids to tokens
